...
{
	DATA_BUS=((command&0b11110000));//getting the first four bits to be send
	CTL_BUS &=~(1<<LCD_RS);//to send command
	...
	//second nibble
	DATA_BUS=((command&0b00001111)<<4);//sending the second four bits
	CTL_BUS |=(1<<LCD_EN);
	_delay_ms(5);
	CTL_BUS &=~(1<<LCD_EN);
	_delay_ms(5);
...
void lcd_write_character(uint8_t character)
{
	...
	CTL_BUS|=(1<<LCD_RS); //set high to write data
	CTL_BUS |=(1<<LCD_EN);//to write data
	_delay_ms(5);
	CTL_BUS &=~(1<<LCD_EN);//end of sending first half of the data
	_delay_ms(5);
	DATA_BUS=((character & 0b00001111)<<4);//sending the second four bits of the data
	CTL_BUS |=(1<<LCD_EN);
	_delay_ms(5);
	CTL_BUS &=~(1<<LCD_EN);
	_delay_ms(5);
